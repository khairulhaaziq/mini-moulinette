...
int test1(void)
{
	...
	g_call_count = 0;
	btree_apply_suffix(root, &moul_record);
	...
	if (!ok)
		printf("    " RED "[1] btree_apply_suffix() did not visit nodes in suffix order (left, right, root)\n" DEFAULT);
	else
		printf("  " GREEN CHECKMARK GREY " [1] btree_apply_suffix() visited nodes in suffix order (left, right, root)\n" DEFAULT);
	moul_free_tree(root);
	return (ok ? 0 : -1);
}
